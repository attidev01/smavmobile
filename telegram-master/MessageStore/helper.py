            ...
            if whatsapp.alertStatus == "ON":
                ...
                if int(data['gas']) <= 49:
                    send_whatsapp('gas', whatsapp.phone,
                                  whatsapp.language, device, item)
                if float(data['machine']) >= whatsapp.machineVoltage and whatsapp.lastmessage == 0:
                    send_whatsapp('start', whatsapp.phone,
                                  whatsapp.language, device, item)
                    client.lastmessage = 1
                    client.save()
                if float(data['machine']) < whatsapp.machineVoltage and whatsapp.lastmessage == 1:
                    send_whatsapp('stop', whatsapp.phone,
                                  whatsapp.language, device, item)
                    client.lastmessage = 0
                    client.save()
                if int(data['T1']) < whatsapp.temperatureAlert and float(data['machine']) < whatsapp.machineVoltage and whatsapp.lastmessage == 0:
                    send_whatsapp('machine_not_start', whatsapp.phone,
                                  whatsapp.language, device, item)
                if int(data['T1']) < whatsapp.temperatureAlert and float(data['machine']) < whatsapp.machineVoltage and whatsapp.lastmessage == 1:
